...
void Transaktion::skrivUt(ostream &os) {
    ...
    ostringstream formattera;

    formattera << setw(8) << left << datum << setw(8) << left << typ << setw(10) << left << namn << setw(9) << left << std::setprecision(6) << belopp <<  left << std::to_string(antal_kompisar) << " ";   

    // Om det är kompisar så lägger vi öven till dem i det som ska ut
    if (antal_kompisar > 0) {
        for (int i = 0; i < antal_kompisar; i++) {
            formattera << string(kompisar[i] + " ");
        }
    }

    string lineToPrint = formattera.str();
...
void Transaktion::skrivTitel(ostream &os) {
    os << setw(8) << left << "Datum" << setw(8) << left << "Typ" << setw(10) << left << "Namn" << setw(9) << left << "Belopp" <<  left << "Antal och lista av kompisar" << endl;
...
PersonLista TransaktionsLista::FixaPersoner() {
    ...
    for (int i = 0; i < antal_transaktioner; i++) {
        ...
        for (int j = 0; j < antalKompisar; j++) {
            string kompisnamn = transaktion.hamtaKompisNamn(j);
